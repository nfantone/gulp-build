...
var Builder = (function() {
  ...
  GulpBuilder.prototype.fonts = function fonts() {
    ...
    var fontsSrc = [].concat(self.config.assets.fonts, bowerFonts);
  ...
  GulpBuilder.prototype.optimize = function optimize() {
    ...
    var jsAppFilter = $.filter('**/' + self.config.optimize.app, { restore: true });
    var jslibFilter = $.filter('**/' + self.config.optimize.vendors, { restore: true });
    var customFilter = $.filter([].concat('*', self.config.optimize.filter));
