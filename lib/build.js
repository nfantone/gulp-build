...
var $ = require('gulp-load-plugins')({rename: {'gulp-log2': 'log'}});
...
var GulpBuilder = (function () {
	 ...
	 */
	GulpBuilder.prototype.cleanFiles = function (path) {
		$.log('Cleaning: ' + path);
		del.sync(path);
	};
	...
	GulpBuilder.prototype.wiredep = function wiredep() {
		...
		return gulp
			...
			.pipe($.log('Wiring "main" bower dependencies into html', {title: 'gulp-build'}))
	...
	GulpBuilder.prototype.injectCSS = function injectcss() {
		return gulp
			.src(self.config.src.index)
			.pipe($.log('Wiring compiled css into html', {title: 'gulp-build'}))
			.pipe($.inject(gulp.src(path.join(self.config.temp, '**/*.css'),
				{read: false}), {ignorePath: self.config.temp}))
			.pipe(gulp.dest(self.config.src.root));
	};
	...
	GulpBuilder.prototype.styles = function styles() {
		...
		return gulp
			.src(self.config.src.less)
			.pipe($.log('Compiling .less -> .css', {title: 'gulp-build'}))
	...
	GulpBuilder.prototype.fonts = function fonts() {
		...
		var bowerFonts = require('main-bower-files')({filter: '**/*.{eot,svg,ttf,woff,woff2}'});
		var fonts = [].concat(self.config.assets.fonts, bowerFonts);

		return gulp
			.src(fonts)
			.pipe($.rev())
			.pipe($.log('Moving fonts', {title: 'gulp-build'}))
	...
	GulpBuilder.prototype.images = function images() {
		...
		return gulp
			.src(self.config.assets.images, {base: self.config.assets.root})
			.pipe($.rev())
			.pipe($.log('Optimizing and moving images', {title: 'gulp-build'}))
	...
	GulpBuilder.prototype.templateCache = function templatecache() {
		var templateOptions = _.defaults(self.config.templateCache.options, {root: self.config.src.root});
		self.cleanFiles(path.join(self.config.temp, '/**/*.js'));

		return gulp
			.src([self.config.src.html, '!' + self.config.src.index])
	...
	GulpBuilder.prototype.injectTemplateCache = function injecttemplates() {
		...
		return gulp
			.src(self.config.src.index)
			.pipe($.log('Wiring templates into html', {title: 'gulp-build'}))
			.pipe($.inject(gulp.src(templateCache, {read: false}),
				{name: 'inject:templates', ignorePath: self.config.temp}))
			.pipe(gulp.dest(self.config.src.root));
	};
