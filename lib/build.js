...
const rimport = require('rework-import');
...
var Builder = (function() {
  ...
  GulpBuilder.prototype.fonts = function fonts() {
    ...
    return gulp
      ...
      .pipe($.flatten())
      .pipe(gulp.dest(path.join(self.config.build, 'fonts')))
  ...
  GulpBuilder.prototype.optimize = function optimize() {
    ...
    return gulp
      ...
      // Get the css
      .pipe(cssFilter)
      // Apply gulp-rework custom transformations
      .pipe($.if(!_.isEmpty(self.config.rework), $.rework.apply(null, self.config.rework)))
      .pipe($.csso())
  ...
  GulpBuilder.prototype.resources = function resources() {
    var resourcesAsArray = _.castArray(self.config.resources);
    var merged = MergeStream();
    resourcesAsArray.forEach((res) => {
      let stream;
      if (_.isObject(res)) {
        stream = gulp.src(res.patterns, {
          base: res.root
        });
      } else {
        stream = gulp.src(res);
  ...
  /**
   * Cleanup temporal resources after finishing build.
   * @method function
   * @param  {Function} cb Gulp callback
   * @return {Stream}
   */
  GulpBuilder.prototype.cleanup = function cleanup(cb) {
    self.cleanFiles([
      path.join(self.config.build, 'rev-manifest.json'),
      path.join(self.config.temp)
    ]);
    return cb();
  };

  GulpBuilder.prototype.package = function() {
    var pck = require(path.resolve(path.join(self.config.root, 'package.json')));
    var artifact = `${pck.name}-${pck.version}`;
    self.cleanFiles(path.join(self.config.build, `${artifact}.tar.gz`));

    return gulp
      .src(path.join(self.config.build, '**'))
      .pipe($.tar(`${artifact}.tar`))
      .pipe($.gzip())
      .pipe($.log(`Generating artifact ${artifact}.tar.gz`, {
        title: 'gulp-build'
      }))
      .pipe(gulp.dest(self.config.root));
  };
